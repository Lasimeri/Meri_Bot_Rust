...
use crate::commands::admin::{RESTART_COMMAND, SHUTDOWN_COMMAND, ADMINHELP_COMMAND, FORCERESTART_COMMAND, DIAGNOSE_COMMAND};
...
#[group]
#[commands(ping, echo, lm, clearcontext, clearreasoncontext, reason, sum, rank, help, lmhelp, reasonhelp, restart, shutdown, adminhelp, forcerestart, diagnose)]
struct General;
...
#[tokio::main]
async fn main() {
    // Enable info and above log levels (trace logging disabled)
    ...
    // Use info level to show info, warn, and error logs (trace and debug disabled)
    // Output to log.txt file for debugging
    ...
    log::info!("Log level: INFO (trace logging disabled)");
